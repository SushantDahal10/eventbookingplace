...
const TicketModal = ({ booking, onClose }) => {
    ...
    const eventDate = booking.events?.event_date ? new Date(booking.events.event_date).toLocaleDateString() : 'Date';
                                    ...
                                    <div key={booking.id} className="group relative bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col">
                                        <div className="h-40 relative cursor-pointer bg-secondary" onClick={() => setSelectedTicket(booking)}>
                                            ...
                                            <div className="absolute inset-0 flex items-end p-6">
                                                <div>
                                                    <h3 className="text-white font-bold text-xl leading-tight mb-1">{eventTitle}</h3>
                                                    <p className="text-white/80 text-xs font-medium uppercase tracking-wider">{eventLocation}</p>
                                                </div>
                                            </div>
                                            <div className={`absolute top-4 right-4 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest backdrop-blur-md ${isPaid ? 'bg-green-500/20 text-green-100 border border-green-400/30' : 'bg-yellow-500/20 text-yellow-100 border border-yellow-400/30'}`}>
                                                {booking.status}
                                            </div>
                                        </div>
